...
def test_conftest_command_covers_application_helpers(db_session, monkeypatch):
    ...
    valid_data = {
        "first_name": "Valid",
        "last_name": "User",
        "email": "valid_conftest_schema@example.com",
        "username": "validconftestschema",
        "password": "Password1",
    }
    assert UserCreate.model_validate(valid_data).password == "Password1"
    assert UserLogin.model_validate({"username": "validconftestschema", "password": "Password1"}).username == "validconftestschema"
    ...
    user_data = {
        "first_name": "Conftest",
        "last_name": "User",
        "email": f"conftest_user_{suffix}@example.com",
        "username": f"conftest_user_{suffix}",
        "password": password,
    }
    ...
    assert repr(user) == f"<User(name=Conftest User, email={user.email})>"
    ...
    duplicate_data = user_data | {
        "username": f"conftest_duplicate_{suffix}",
        "password": "Password1",
    }
    ...
    invalid_schema_data = user_data | {
        "email": "not-an-email",
        "username": f"conftest_invalid_{suffix}",
        "password": "Password1",
    }
